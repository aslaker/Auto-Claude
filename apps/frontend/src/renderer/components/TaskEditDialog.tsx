...
import { Loader2, Image as ImageIcon, ChevronDown, ChevronUp, X, Users } from 'lucide-react';
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
import type { Task, ImageAttachment, TaskCategory, TaskPriority, TaskComplexity, TaskImpact, ModelType, ThinkingLevel, PersonaAlignment } from '../../shared/types';
import {
...
export function TaskEditDialog({ task, open, onOpenChange, onSaved }: TaskEditDialogProps) {
  ...
  useEffect(() => {
    if (open) {
      ...
      setTargetPersonaIds(task.metadata?.targetPersonaIds || []);
      setPersonaAlignment(task.metadata?.personaAlignment || []);
      setError(null);
      ...
      // Auto-expand persona section if task has personas
      setShowPersonaSection((task.metadata?.targetPersonaIds || []).length > 0);
  ...
  const handleSave = async () => {
    ...
    const hasChanges =
      ...
      JSON.stringify(targetPersonaIds) !== JSON.stringify(task.metadata?.targetPersonaIds || []) ||
      JSON.stringify(personaAlignment) !== JSON.stringify(task.metadata?.personaAlignment || []);
    ...
    // Persona targeting - always set to allow clearing personas
    metadataUpdates.targetPersonaIds = targetPersonaIds.length > 0 ? targetPersonaIds : undefined;
    metadataUpdates.personaAlignment = personaAlignment.length > 0 ? personaAlignment : undefined;
      ...
      <DialogContent className="sm:max-w-[550px] max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-5 py-4">
          ...
          <button
            ...
            aria-controls="edit-persona-section"
          >
